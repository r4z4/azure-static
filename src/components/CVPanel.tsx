...
export interface CVPanelProps {
    panelData: CVPanelData;
}

export interface CVPanelData {
    name: String;
    date: String;
    desc: String;
    img?: string;
    bgColor?: string;
    category: String;
    documents: CVPanelDocument[];
}

export interface CVPanelDocument {
    id: number;
    filename: String;
    url: string;
...
function CVPanel({ panelData }: CVPanelProps) {
    ...
    <div className="collapse-panel" style={{backgroundColor: panelData.bgColor ? panelData.bgColor : ''}}>
      ...
      {expanded ? (
        <div className="expandable">
          {panelData.documents.map((doc: CVPanelDocument) => (
